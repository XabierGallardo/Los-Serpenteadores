...
docGEBI("boton-api").addEventListener("click", () => {
...
function load_info() {
    const listDivs = doc.createDocumentFragment();
    for (const book of books) {
        let text = '';
        text += `    NOMBRE => ${book.name}\n`;
        text += `   PAGINAS => ${book.numberOfPages}\n`;
        text += `PUBLICADOR => ${book.publisher}\n`;
        text += `    ORIGEN => ${book.country}\n`;
        text += `   ESTRENO => ${book.released.split('T')[0]}\n`;
        text += `       URL => ${book.url}`;

        const divAnimation = doc.createElement('DIV');
        divAnimation.setAttribute('class', 'animation__div');
        divAnimation.setAttribute('data-id', book.url.split('/').at(-1));

        const btn_actionAnimation = doc.createElement('BUTTON');
        btn_actionAnimation.onclick = (e) => animation(e);
        btn_actionAnimation.setAttribute('class', 'btn__Animation');
        btn_actionAnimation.setAttribute('type', 'button');
        btn_actionAnimation.append('INICIAR / FINALIZAR ANIMACIÓN');

        const elementPRE = doc.createElement('PRE');
        elementPRE.setAttribute('class', 'info__pre');
        elementPRE.append(text);

        divAnimation.append(
            elementPRE, btn_actionAnimation
        );
        listDivs.append(divAnimation);

        log(text);
    };
    const sectionAnimations = docGEBI('animations__section');
    sectionAnimations.append(listDivs);
};
doc.addEventListener('DOMContentLoaded', load_info);

function animation({ path }) {
    path = path[1];
    toggleClass(path, path.dataset.id);
    path.childNodes.forEach(node => { toggleClass(node, path.dataset.id); });
};

function toggleClass(node, numberDIV) {
    node.classList.toggle(`Animation_${numberDIV}`);
    node.removeEventListener(
        'animationend', () => toggleClass(node, numberDIV)
    );
    log(node.onanimationend);
};